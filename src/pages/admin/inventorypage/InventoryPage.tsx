...
export default function InventoryPage(){
  ...
  const load=async()=>{
    ...
    const [p,s]=await Promise.all([
      fetch(`${API_URL}/api/products/admin`,{headers:auth}),
    ...
    setProducts(Array.isArray(pb)?pb:[]);
    setSummary(Array.isArray(sb)?sb:[]);
  };
  useEffect(()=>{void load();},[]);

  const summaryMap=useMemo(()=>new Map(summary.map(x=>[x.productId,x])),[summary]);
  const filtered=useMemo(()=>{const q=search.trim().toLowerCase();return products.filter(p=>!q||`${p.brand} ${p.model} ${p.storage}`.toLowerCase().includes(q));},[products,search]);
