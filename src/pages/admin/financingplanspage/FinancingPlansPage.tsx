    ...
    <section className="min-h-screen bg-[#f5f5f7] px-4 py-8 md:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-5 overflow-hidden rounded-[28px] bg-white shadow-sm ring-1 ring-black/5">
          <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-[#fafafa] text-xs uppercase tracking-wide text-black/35"><tr><th className="px-6 py-4">Plan</th><th className="px-6 py-4">Plazo</th><th className="px-6 py-4">Tasa anual</th><th className="px-6 py-4">Enganche mín.</th><th className="px-6 py-4">Prioridad</th><th className="px-6 py-4">Estado</th><th className="px-6 py-4">Acción</th></tr></thead><tbody className="divide-y divide-black/5">{plans.slice((page-1)*pageSize,page*pageSize).map(plan=><tr key={plan.id}><td className="px-6 py-4"><p className="font-medium">{plan.name}</p><p className="mt-1 text-xs text-black/35">{plan.description||"Sin descripción"}</p></td><td className="px-6 py-4">{plan.termMonths} meses</td><td className="px-6 py-4">{(Number(plan.annualInterestRate)*100).toFixed(2)}%</td><td className="px-6 py-4">{(Number(plan.minimumDownPaymentRatio)*100).toFixed(2)}%</td><td className="px-6 py-4">{plan.priority}</td><td className="px-6 py-4"><button onClick={()=>void togglePlan(plan)} className={`rounded-full px-3 py-1.5 text-xs font-medium ${plan.active?"bg-black text-white":"bg-black/5 text-black/45"}`}>{plan.active?"Activo":"Inactivo"}</button></td><td className="px-6 py-4"><button onClick={()=>setSelected(plan)} className="inline-flex items-center gap-2 rounded-full bg-[#f5f5f7] px-3 py-2 text-xs"><Eye size={14}/> Ver</button></td></tr>)}{!plans.length&&<tr><td colSpan={7} className="px-6 py-12 text-center text-black/35">Todavía no hay planes configurados.</td></tr>}</tbody></table></div><Pagination page={page} total={plans.length} pageSize={pageSize} onPageChange={setPage}/>
        </div>
      </div>
